...
mjtResult mj_respond(void)
{
    ...
    switch (lastError) {
        ...
        case mjCOM_GETSQUARES   :
            return mj_respond_squares();
            break;
...
mjtResult mj_request_squares(const mjRequestSquares* rqs, mjReplySquares* res)
{
    ...
    // exchange
    mjSend(mjCOM_GETSQUARES, 1*sizeof(int) + (rqs->n)*sizeof(float));
    mjRecv();
...
{
    mjSetError(mjCOM_OK);
    mjRequestSquares rqs;
    memset(&rqs, 0, sizeof(mjRequestSquares));
    mjReplySquares res;
    memset(&res, 0, sizeof(mjReplySquares));
